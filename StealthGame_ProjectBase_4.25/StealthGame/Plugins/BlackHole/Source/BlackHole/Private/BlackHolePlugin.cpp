...
// Sets default values
ABlackHolePlugin::ABlackHolePlugin()
{
	...
	// Bind to Event
	InnerSphereComponent->OnComponentBeginOverlap.AddDynamic(this, &ABlackHolePlugin::OverlapInnerSphere);
...
void ABlackHolePlugin::OverlapInnerSphere(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp,
...
// Called every frame
void ABlackHolePlugin::Tick(float DeltaTime)
{
	...
	for (int32 i = 0; i < OverlappingComps.Num(); i++) {
		...
		if (PrimComp && PrimComp->IsSimulatingPhysics()) {
			// This will simulate the applied forces
			const float SphereRadius = OuterSphereComponent->GetScaledSphereRadius();
			const float ForceStrength = -2000; // This value is negative to make it pull towards the origin instead of pushing it away
